...
function propertiesValid(passport) {
    ...
    if (!passport.hasOwnProperty('hgt') ||
        !(/^\d+(cm|in)$/.test(passport.hgt.trim()))
    ) {
        console.log(`height failed: ${passport.hgt}`);
        return false;
    }

    if (passport.hgt.includes('cm')) {
        if (parseInt(passport.hgt) < 150 || parseInt(passport.hgt) > 193) {
    ...
    if (passport.hgt.includes('in'))
        if (parseInt(passport.hgt) < 59 || parseInt(passport.hgt) > 76) {
